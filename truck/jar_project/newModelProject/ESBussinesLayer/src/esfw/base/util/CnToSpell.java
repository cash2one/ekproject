...
//        System.out.println("." + asciiRang0 + "..." + ascii + "." + asciiRang);
        if (ascii >= asciiRang0 && ascii < asciiRang) { //区间找到
//          System.out.println("区间找到了..");
  ...
  /**
   * 转换姓名->保证姓名后面字符不是数字(去掉姓名后面的数字)
   * @param cnStr String
   * @return String
   */
  public static String trimRightNumber(String cnStr) {
    if (null == cnStr || "".equals(cnStr.trim())) {
      System.out.println("==转换对象为空==");
      return cnStr;
    }
    String tmp = "";
    for (; ; ) {
      ...
      tmp = cnStr.substring(cnStr.length() - 1, cnStr.length());
      try {
        Integer.parseInt(tmp);
        cnStr = cnStr.substring(0, cnStr.length() - 1);
      }
      catch (Exception e) {
        return cnStr.substring(0, cnStr.length());
      }
    }
  }
  ...
  public static String getFullSpell(String cnStr) {
    ...
    //转换姓名
    cnStr = trimRightNumber(cnStr);
    char[] chars = cnStr.toCharArray();
    StringBuffer retuBuf = new StringBuffer();
    for (int i = 0, Len = chars.length; i < Len; i++) {
//      System.out.print(":" + chars[i] + ":");
      int ascii = getCnAscii(chars[i]);
//      System.out.println("取ascii值:" + ascii);
      if (ascii == 0) { //取ascii时出错
        retuBuf.append(""); //060918 modify by yhl
      }
      else {
...
//        System.out.println("***" + spell);
  ...
  public static void main(String args[]){
	  String cn="苏俊";
	  System.out.println("yue"+getFullSpell(cn));
	  
	  /*char[] chars = cn.toCharArray();
	  for(char i:chars){
		  System.out.println("char:"+i);
		 int ac= CnToSpell.getCnAscii(i);
		  System.out.println("ac:"+ac);
		  System.out.println("spell:"+getSpellByAscii(ac));
	  }*/
  }
}
