 ...
 * 
 */
public class TransBooklogDao extends AbstractTransDao implements
		TransQueryDao<TransBooklogParams, TransBooklogRow> {
	...
	public List<TransBooklogRow> query(TransBooklogParams requsetParams) {
		...
		for (String areaAbb : serviceAreas) {
			if (this.isPackageArea(areaAbb)) {
				tempSet = this.packageTransBookLog(phone, areaAbb, studentName,
						beginDate, endDate);
			} else
				tempSet = this.baseTransBookLog(phone, areaAbb, studentName,
						beginDate, endDate);
	 ...
	 * @return
	 */
	List<TransBooklogRow> baseTransBookLog(String phone, String areaAbb,
			String studentName, String beginDate, String endDate) {
		List<TransBooklogRow> rows = new ArrayList<TransBooklogRow>();
		BaseDao db = null;
		ResultSet rs = null;
		try {
			db = new BaseDao(POOL_NAME, DBConnector.getConnection(POOL_NAME));
			rs = db.queryByPage(queryBaseTranscationLogsSql(phone, areaAbb,
					studentName, beginDate, endDate), 1, record_max_limit);
			TransBooklogRow row = null;
			while (rs != null && rs.next()) {
				if (rs.getInt("transaction") > 0
						&& rs.getInt("transaction") < 5) {
					row = new TransBooklogRow();
					row.setTransaction(TransactionType.values()[rs
							.getInt("transaction") - 1].cname());
				} else
					continue;
				...
				row.setOperationType(rs.getInt("open") == 0 ? "取消" : (rs
						.getInt("open") == 1 ? "开通" : "修改收费"));
				row.setRemark("备注信息");
	 ...
	 * @return
	 */
	String queryBaseTranscationLogsSql(String phone, String areaAbb,
			String studentName, String beginDate, String endDate) {
		StringBuffer mainSql = new StringBuffer();
		mainSql
				.append(" select xj_school.school_name,cla.class_name,stu.name stu_name,fa.name parent_name,tlog.transaction,tlog.open,book_type,reason,operator,open_date ");
		...
		mainSql
				.append(" left join xj_class cla on cla.id=scla.class_id and cla.school_id=scla.school_id ");
		...
		if (!Checker.isNull(studentName)
				&& ("全部".equals(studentName) || "all".equals(studentName
						.toLowerCase())))
			mainSql.append(" and stu.name like '%" + studentName + "%'");
		if (!Checker.isNull(phone))
			mainSql.append(" and( fa.phone like '" + phone
					+ "'or fa.kf_phone like '" + phone + "')");
	 ...
	 * @return
	 */
	List<TransBooklogRow> packageTransBookLog(String phone, String areaAbb,
			String studentName, String beginDate, String endDate) {
		List<TransBooklogRow> rows = new ArrayList<TransBooklogRow>();
		BaseDao db = null;
		ResultSet rs = null;
		try {
			db = new BaseDao(POOL_NAME, DBConnector.getConnection(POOL_NAME));
			rs = db.queryByPage(queryPackageTranscationLogsSql(phone, areaAbb,
					studentName, beginDate, endDate), 1, record_max_limit);
			TransBooklogRow row = null;
			while (rs != null && rs.next()) {
	 ...
	 * @return
	 */
	String queryPackageTranscationLogsSql(String phone, String areaAbb,
			String studentName, String beginDate, String endDate) {
		StringBuffer mainSql = new StringBuffer();
		mainSql
				.append(" select xj_school.school_name,cla.class_name,stu.name stu_name,fa.name parent_name,pp.name transaction,pp.remark,tlog.open,book_type,reason,operator,open_date ");
		mainSql.append(" from ").append(areaAbb + "_transaction_log tlog ");
