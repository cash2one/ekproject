...
//			printDatas(items);
			...
			dao.insert(items);
			AppLoger.getRuningLogger().info("已经执行入库操作。");
			
			//调用邮件服务接口
			String messageBody = dao.wrapperEMailMessageContent();
			if(messageBody!=null&&!"".equals(messageBody)){
				AppLoger.getRuningLogger().info("需要发送通知邮件....");
				MailBoxService mailBox = new MailBoxService(SysCfg.EMAIL_FROM_ADDRESS,
						SysCfg.EMAIL_STMP_HOST, SysCfg.EMAIL_USER_NAME, SysCfg.EMAIL_USER_PWD, SysCfg.EMAIL_AUTH,true);
				try{
				   AppLoger.getRuningLogger().info("准备送出一封通知处理邮件["+SysCfg.EMAIL_TO_ADDRESS+"]。");
...
//			String queryTimePer = "2011-02-18";
	...
	void getComplaintDetailInfos(HttpClient client,ComplaintItem item) throws HttpException, IOException{
		...
		if(hasReOpenReplyMsg!=null&&hasReOpenReplyMsg.length()>0)
			item.setContent("用户:"+item.getUser()+" ， "+hasReOpenReplyMsg);
