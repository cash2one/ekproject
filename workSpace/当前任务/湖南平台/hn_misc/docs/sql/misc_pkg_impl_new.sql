CREATE OR REPLACE package body HNXXT.misc_pkg as
	...
	/* default password for new cp user */
	v_DEFAULT_PASSWORD CONSTANT VARCHAR2(30 CHAR) := '123456';
    ...
    */
    procedure deal_cp_order_relation(
		...
		/* 
			check if user has order this transaction
			
			return the cp_tran_customer id if user has ordered,else return 0
		*/
        ...
        begin
            ...
            if v_customer_id = 0 then --用户不存在
                insert into cp_customer(userid,password,phone,add_date,res_psw,operator)
                    values(p_phone,v_DEFAULT_PASSWORD,p_phone,sysdate,to_char(trunc(dbms_random.value(100000,999999))),'MISC')
                    returning id into v_customer_id;
            end if;
            
            v_rel_id := has_order(p_phone,p_tran_info.salemodalid);
			
			if v_rel_id > 0 then 
				--has order this transaction,do update
				update cp_tran_customer set operate_date = sysdate where id = v_rel_id;
			else
				--add a new relation
        ...
        begin
            ...
            if sql%found then
				--the delete opration is success, log the opration
